...
public class BootUp implements Job{
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		cn.edu.cuit.icloud.call.virtualmachine.start.RequestParameters requestParameters = new cn.edu.cuit.icloud.call.virtualmachine.start.RequestParameters();
		...
		if(null != list) {
			list.forEach(s->{
				...
				try {
					String json = callService.callCommand2(Cmd.START_VIRTUALMACHINE, gson.toJson(requestParameters));
					list2.add(json);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
		...
		System.out.println("开启了；"+list2.size()+"台虚拟机.");
